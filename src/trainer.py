...
class Trainer(object):
    ...
    def get_dis_AB(self, z, volatile):
        """
        Get discriminator input batch / output target (A->B)
        Output: x: [G(z_x, z), z_y], y: [0.8, 0.2], orig_src: x, orig_p: z_x
        """
        ...
        p = self.encoder_A(src_emb)     # z_x
        q = self.encoder_B(tgt_emb)     # z_y
        orig_p = (p.clone()).data
        src_emb = self.mapping_G(p.data, z) if self.params.mid_domain else self.mapping_G(p.data) # z_x --> z*G(z_x)+(1-z)*G(z_x)
        tgt_emb = q.data
    ...
    def get_dis_BA(self, z, volatile):
        """
        Get discriminator input batch / output target (B->A)
        Output: x: [F(z_y, z), z_x], y:[0.8, 0.2], orig_tgt: y, orig_q: z_y
        """
        # select random word IDs
        bs = self.params.batch_size
        mf1 = self.params.dis_most_frequent_AB
        ...
        orig_q = (q.clone()).data
        tgt_emb = self.mapping_F(q.data, z) if self.params.mid_domain else self.mapping_F(q.data)  ## z_y --> F(enc(z_y))
    ...
    def get_dis_BB(self, z, volatile):
        """
        Get discriminator input batch / output target (B->A)
        Output: x: [F(z_y2, z), z_y1], y: [0.8, 0.2], orig_tgt: y2, orig_q: z_y2
        """
        # select random word IDs
        bs = self.params.batch_size
        mf2 = self.params.dis_most_frequent_BA
        assert mf2 <= min(len(self.src_dico), len(self.tgt_dico))
        tgt_ids1 = torch.LongTensor(bs).random_(len(self.tgt_dico) if mf2 == 0 else mf2)
        tgt_ids2 = torch.LongTensor(bs).random_(len(self.tgt_dico) if mf2 == 0 else mf2)
        if self.params.cuda:
            tgt_ids1 = tgt_ids1.cuda()
            tgt_ids2 = tgt_ids2.cuda()

        # get word embeddings
        tgt_emb1 = self.tgt_emb(tgt_ids1)
        tgt_emb2 = self.tgt_emb(tgt_ids2)

        p = self.encoder_B(tgt_emb1)
        q = self.encoder_B(tgt_emb2)
        tgt_emb1 = p.data
        orig_tgt = (tgt_emb2.clone()).data
        orig_q = (q.clone()).data
        tgt_emb2 = self.mapping_F(q.data, z) if self.params.mid_domain else self.mapping_F(q.data)  ## z_y --> F(enc(z_y))

        # input / target
        x = torch.cat([tgt_emb2, tgt_emb1], 0)
        y = torch.FloatTensor(2 * bs).zero_()
        y[:bs] = 1 - self.params.dis_smooth
        y[bs:] = self.params.dis_smooth
        y = y.cuda() if self.params.cuda else y
    ...
    def get_dis_AA(self, z, volatile):
        """
        Get discriminator input batch / output target (A->B)
        Output: x: [G(z_x2, z), z_x1], y: [0.8, 0.2], orig_tgt: x2, orig_q: z_x2
        """
        # select random word IDs
        bs = self.params.batch_size
        mf1 = self.params.dis_most_frequent_AB
        assert mf1 <= min(len(self.src_dico), len(self.tgt_dico))
        src_ids1 = torch.LongTensor(bs).random_(len(self.src_dico) if mf1 == 0 else mf1)
        src_ids2 = torch.LongTensor(bs).random_(len(self.src_dico) if mf1 == 0 else mf1)
        if self.params.cuda:
            src_ids1 = src_ids1.cuda()
            src_ids2 = src_ids2.cuda()

        # get word embeddings
        src_emb1 = self.src_emb(src_ids1)
        src_emb2 = self.src_emb(src_ids2)

        p = self.encoder_A(src_emb1)
        q = self.encoder_A(src_emb2)
        src_emb1 = p.data
        orig_tgt = (src_emb2.clone()).data
        orig_q = (q.clone()).data
        src_emb2 = self.mapping_G(q.data, z) if self.params.mid_domain else self.mapping_G(q.data)  ## z_y --> F(enc(z_y))

        # input / target
        x = torch.cat([src_emb2, src_emb1], 0)
        y = torch.FloatTensor(2 * bs).zero_()
        y[:bs] = 1 - self.params.dis_smooth
        y[bs:] = self.params.dis_smooth
        y = y.cuda() if self.params.cuda else y
    ...
    def dis_step_B(self, stats):
        ...
        # loss
        bs = self.params.batch_size
        z = stats['z'] if self.params.mid_domain else 1.0
        x, y, orig_A, orig_p = self.get_dis_AB(z, volatile=True)
        # x: [G(z_x, z), z_y], y: [0.8, 0.2], orig_A: x, orig_p: z_x
        preds = self.discriminator_B(x.data)  # preds: [D_B(G(z_x, z)), D_B(z_y)]
        loss = F.binary_cross_entropy(preds, y)

        if self.params.mid_domain:
            x, y, orig_A, orig_p = self.get_dis_BB(z, volatile=True)
            # x: [F(z_y2, z), z_y1], y: [0.8, 0.2], orig_A: y2, orig_p: z_y2
            preds = self.discriminator_B(x.data)  # preds: [D_B(F(z_y2, z)), D_B(z_y1)]
            y[:bs] = z
            loss = z*loss + (1-z)*F.binary_cross_entropy(preds, y)
    ...
    def dis_step_A(self, stats):
        ...
        # loss
        bs = self.params.batch_size
        # z = stats['z'] if self.params.mid_domain or self.params.mixup else 1.0
        z = stats['z'] if self.params.mid_domain else 1.0
        x, y, orig_B, orig_q = self.get_dis_BA(z, volatile=True)
        # x: [F(z_y, z), z_x], y: [0.8, 0.2], orig_tgt: y, orig_q: z_y
        preds = self.discriminator_A(x.data)  # preds: [D_A(F(z_y, z)), D_A(z_x)]
        loss = F.binary_cross_entropy(preds, y)

        if self.params.mid_domain:
            x, y, orig_A, orig_p = self.get_dis_AA(z, volatile=True)
            # x: [G(z_x2, z), z_x1], y: [0.8, 0.2], orig_A: x2, orig_p: z_x2
            preds = self.discriminator_A(x.data)  # preds: [D_A(G(z_x2, z), D_A(z_x1)]
            y[:bs] = z
            loss = z*loss + (1 - z)*F.binary_cross_entropy(preds, y)
    ...
    def mapping_step_G(self, stats):
        ...
        # z = stats['z'] if self.params.mid_domain or self.params.mixup else 1.0
        z = stats['z'] if self.params.mid_domain else 1.0
        x, y, orig_A, orig_p = self.get_dis_AB(z, volatile=False)
        # x: [G(z_x, z), z_y], y: [0.8, 0.2], orig_A: x, orig_p: z_x
        preds = self.discriminator_B(x)  # preds: [D_B(G(z_x, z)), D_B(z_y)]
        loss = F.binary_cross_entropy(preds, 1 - y)

        if self.params.mid_domain:
            x, y, orig_A, orig_p = self.get_dis_AA(z, volatile=True)
            # x: [G(z_x2, z), z_x1], y: [0.8, 0.2], orig_A: x2, orig_p: z_x2
            preds = self.discriminator_A(x)  # preds: [D_A(G(z_x2, z)), D_A(z_x1)]
            y[:bs] = z
            loss = z*loss + (1 - z)*F.binary_cross_entropy(preds, 1 - y)
        ...
        bs = self.params.batch_size
        p_hat = self.mapping_F(x.data[:bs], z) if self.params.mid_domain else self.mapping_F(x.data[:bs])
    ...
    def mapping_step_F(self, stats):
        ...
        bs = self.params.batch_size
        z = stats['z'] if self.params.mid_domain else 1.0
        x, y, orig_B, orig_q = self.get_dis_BA(z, volatile=False)
        # x: [F(z_y, z), z_x], y: [0.8, 0.2], orig_B: y, orig_q: z_y
        preds = self.discriminator_A(x)  # preds: [D_A(F(z_y, z)), D_A(z_x)]
        loss = F.binary_cross_entropy(preds, 1 - y)

        if self.params.mid_domain:
            x, y, orig_B, orig_q = self.get_dis_BB(z, volatile=True)
            # x: [F(z_y2, z), z_y1], y: [0.8, 0.2], orig_A: y2, orig_p: z_y2
            preds = self.discriminator_B(x)  # preds: [D_B(F(z_y2, z)), D_B(z_y1)]
            y[:bs] = z
            loss = z*loss + (1-z)*F.binary_cross_entropy(preds, 1 - y)
        ...
        # Cycle/Back_translation loss
        bs = self.params.batch_size
        q_hat = self.mapping_G(x.data[:bs], z) if self.params.mid_domain else self.mapping_G(x.data[:bs])
    ...
    def build_dictionary_AB(self):
        ...
        """
        if self.params.mid_domain:
            src_emb = self.mapping_G(self.encoder_A(self.src_emb.weight.data), 1).data
        else:
    ...
    def build_dictionary_BA(self):
        ...
        if self.params.mid_domain:
            tgt_emb = self.mapping_F(self.encoder_B(self.tgt_emb.weight.data), 1).data
        else:
    ...
    def procrustes_AB(self):
        ...
        if self.params.mid_domain:
            W = self.mapping_G.generator.weight.data
        else:
    ...
    def procrustes_BA(self):
        ...
        if self.params.mid_domain:
            W = self.mapping_F.generator.weight.data
        else:
    ...
    def save_best_AB(self, to_log, metric_AB):
        ...
        if to_log[metric_AB] > self.best_valid_metric_AB:
            ...
            if self.params.mid_domain:
                W = self.mapping_G.generator.weight.data.cpu().numpy()
            else:
            ...
            torch.save(self.encoder_B.state_dict(), path)
    ...
    def save_best_BA(self, to_log, metric_BA):
        ...
        if to_log[metric_BA] > self.best_valid_metric_BA:
            ...
            if self.params.mid_domain:
                W = self.mapping_F.generator.weight.data.cpu().numpy()
            else:
    ...
    def reload_best_AB(self, path=None):
        ...
        if self.params.mid_domain:
            W1 = self.mapping_G.generator.weight.data
        else:
            W1 = self.mapping_G.weight.data
        # print(to_reload.size())
        # print(W1.size())
        assert to_reload.size() == W1.size()
        W1.copy_(to_reload.type_as(W1))
        
        if "unsup_orig" in self.params.exp_path:
            ## reload enc_X (A->B)
            ...
            # reload enc_X (A->B)
            self.encoder_A.load_state_dict(torch.load(path2))
            # reload enc_Y (A->B)
            self.encoder_B.load_state_dict(torch.load(path3))
    ...
    def reload_best_BA(self, path=None):
        ...
        to_reload = torch.from_numpy(torch.load(path1))
        if self.params.mid_domain:
            W1 = self.mapping_F.generator.weight.data
        else:
