...
# declare -a tgt_lang_list=(ar ar ar bg bg hi tr fr ru de es zh)
# declare -a exp_id_llist=(003 204 201 002 102 011 011 001 011 001 001 019)
declare -a tgt_lang_list=(hi)
declare -a exp_id_llist=(001)
...
exp_name_en_list[$i]='orig/'$tgt_lang'_'$exp_id'_l_en'
exp_name_z_list[$i]='orig/'$tgt_lang'_'$exp_id'_l'
mapping_dir_list[$i]='/mnt/ssd-201-112-01/cpii.local/lptang/checkpoints/CLWE/unsup_orig/en_'$tgt_lang'/'$exp_id
done
...
output_file="/users/cpii.local/lptang/NLP_models/CLWE_formal/output/original_results_orig.txt"
